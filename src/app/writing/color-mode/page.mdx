...
Arguably this is even simpler than using `@property` _and_ it opens up new creative possibilities, such as this [cool circular reveal effect](https://akashhamirwasia.com/blog/full-page-theme-toggle-animation-with-view-transitions-api/) by Akash Hamirwasia. However, because they rely on static screenshots, view transitions can cause visual artifacts, especially if you have any animated elements on the page.
...
Both techniques can be implemented as _progressive enhancement_ - older browsers will simply apply the color change without the animation, so dark mode stays fully functional. The choice comes down to the effect you're going for.
