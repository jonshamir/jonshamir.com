...
We can then toggle this class on the root element using Javascript, for example when a button is clicked:
...
To get a nice animation between the color modes, we can declare a CSS transition for any property using the variables - in this case, `color` and `background-color`. In order to keep the timing consistent, I like to define `--transition-defaults` which I can reuse:
