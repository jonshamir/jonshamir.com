...
For my website, I wanted to have a smooth transition when switching between light and dark modes. You can check out the final result here:
...
When the `.dark` class is added or removed, the browser will automatically update all the elements that use the properties we defined:

<ColorModeToggleDemo isAnimated={false}>
  Click to toggle `.dark` class:
</ColorModeToggleDemo>

To get a nice animation between the color modes, we can declare a transition for any property using the variables. In order to keep the timing consistent, I like to define `--transition-defaults` which I can reuse:
...
<ColorModeToggleDemo isAnimated={true} isGrowing={true}>
  Click to toggle `.dark` class:
</ColorModeToggleDemo>
